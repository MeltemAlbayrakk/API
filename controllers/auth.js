...
const register= async(req,res )=>{
    try {
        ...
        if(user){
            return res.status(500).json({message:"bu email zaten bulunmakta!!"})
        }
...
const login = async(req,res)=>{
    try {
        ...
        const user = await Auth.findOne({email});
        if(!user){
            return res.status(500).json({massage:"Böyle bir kullanıcı bulunamadı...."})
        }
        const comparePassword = await bcrypt.compare(password,user.password)
        if(!comparePassword){
            return res.status(500).json({massage:"Parolanız yanlışşş..."})
        }
